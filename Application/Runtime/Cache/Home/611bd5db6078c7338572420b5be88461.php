...
<body>
	...
	<!-- this page specific styles -->
	<!-- css -->
    <link rel="stylesheet" href="/Git/epm/project/Public/css/compiled/signup.css" type="text/css" media="screen" />
	...
	<div class="row-fluid login-wrapper" style="margin-top:8%;">
		<form action="<?php echo U('Home/Personal/pswReset');?>" method="post">
        <div class="box">
    ...
    //外部插件功能js
    	;!function(){
			...
			$("#oripsw").on("blur",function(){
			    ...
			    $.ajax({ 
			   		url: "<?php echo U('Home/Personal/checkPsw');?>",
			   		type:'POST',
			   		dataType:'json',
			   		data:{'psw':psw},
			   		success: function(data){
			   			if(data == '1'){			   				
			   				layer.tips('√', handle, {tips: [2, 'green']});
			   				validate[0] = true;
			   			}else{
			   				$("#newpsw").val('');
			   				$("#confirmpsw").val('');
			   				layer.tips('密码错误', handle, {tips: [2, 'red']});
			   				handle.val('').focus();
					...
					error:function(){
						layer.tips('验证失败！', handle, {tips: [2, 'red']});
						$("#btnsubmit").attr('disabled','disabled');
					}
				});	
			})
			...
			$("#newpsw").on("blur",function(){
			    ...
			    var psw = $("#oripsw").val();
			    if(pswa === psw ){
			    	layer.tips('密码不可与原密码一样', this, {tips: [2, 'red']});
			    	$(this).val('');
			    }else if(reg.test(pswa)){
			    	layer.tips('√', this, {tips: [2, 'green']});
			    	validate[1] = true;
			    	return ;
			    }else{
			    	layer.tips('密码为6~18位数字、字母！', this, {tips: [2, 'red']});	
			    }
			    $("#newpsw").val('');
			   	$("#confirmpsw").val('');
				$("#btnsubmit").attr('disabled','disabled');
			...
			$("#confirmpsw").on("blur",function(){
			    ...
			    var pswa = $('#newpsw').val();
			    if(pswb == ''){
			    	layer.tips("确认密码不可为空！", $(this), {tips: [2, 'red']});
			    	...
			    	layer.tips("两次密码不一致！", $(this), {tips: [2, 'red']});
			    }
				$("#btnsubmit").attr('disabled','disabled');
			})
